...
magnitude faster than via `JRC2018F` but comes at a small loss in precision: on
average, the direct transform is less than a nanometer off compared to the
long transform.
...
`MANC_mirror_landmarks.csv` contains 1.9k landmarks to compensate for left/right asymmetries in MANC. These landmarks were generated by:
1. Evenly sampling the bounding box of the MANC neuropil
2. Flip the points across the x-axis (`x_flip`, `y_flip`, `z_flip` columns)
3. Mirror the points properly via `JRCVNC2018M` (`x_mirr`, `y_mirr`, `z_mirr` columns)

The code for the generation lives in `MANC_mirror_landmarks.ipynb`.

## `JRCFIB2018F_mirror_landmarks.csv`
`JRCFIB2018F_mirror_landmarks.csv` contains 1.4k landmarks to compensate for left/right asymmetries in JRCFIB2018F (hemibrain). These landmarks were generated by:
1. Evenly sampling the bounding box of the JRCFIB2018F neuropil
2. Flip the points across the x-axis (`x_flip`, `y_flip`, `z_flip` columns)
3. Mirror the points properly via `JRC2018F` (`x_mirr`, `y_mirr`, `z_mirr` columns)

The code for the generation lives in `JRCFIB2018F_mirror_landmarks.ipynb`.
